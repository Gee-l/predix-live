...
export class NodesEndpointService {
  ...
  getAllInfo() {
    ...
    //                 farms_main[farmIndex].nodes.push(new (node.sensors.length > 0 ) ? Rx.from(node.sensors) : return Node(node.uri, node.name, node.manufacturer, node.location, new Array<Sensor>(), node.status, node.description));
